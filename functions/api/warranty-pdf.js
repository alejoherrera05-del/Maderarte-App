...
export async function generateWarrantyPdf(env,document){
  if(document?.documentKind!=='warranty'||document.issued!==true||!document.number||!['RECEPCION','DOMICILIO','ENTREGA'].includes(document.type))throw Object.assign(Error('No se confirmó el comprobante del expediente.'),{code:'DOCUMENT_PLAN_INVALID',status:503});
  ...
  return {name:(document.type==='ENTREGA'?'Entrega-garantia-':document.type==='RECEPCION'?'Recepcion-':'Atencion-domicilio-')+document.number+'.pdf',mime:'application/pdf',base64};
}
