...
function verifySchema_() {
  ...
  Object.keys(REQUIRED_HEADERS).forEach(function(sheetName) {
    var sheet = getSheet_(sheetName), actual = getHeaders_(sheet), expected = REQUIRED_HEADERS[sheetName];
    if (typeof optionalProperty_ === 'function' && ['2', '3'].indexOf(optionalProperty_('ORDER_SCHEMA_VERSION', '1')) >= 0 && typeof ORDER_CREATION_EXTRA_HEADERS_ !== 'undefined') expected = expected.concat(ORDER_CREATION_EXTRA_HEADERS_[sheetName] || []);
    var exact = actual.length === expected.length && actual.every(function(header, index) { return header === expected[index]; });
    if (!exact) throw appError_('SHEET_SCHEMA_MISMATCH', 'La pestaña ' + sheetName + ' no coincide exactamente con el contrato.', 503, { missing: expected.filter(function(header) { return actual.indexOf(header) === -1; }), unexpected: actual.filter(function(header) { return expected.indexOf(header) === -1; }), orderMismatch: actual.length === expected.length });
  });
  return true;
}
...
function publicProfile_(user) {
  return { uid: String(user.UID_Firebase || ''), email: normalizeEmail_(user.Email), name: String(user.Nombre_Completo || ''), role: normalizeCode_(user.Rol), status: normalizeCode_(user.Estado), mainBranch: normalizeCode_(user.Sede_Principal), branches: String(user.Sedes_Permitidas || '').split(',').map(function(item) { return normalizeCode_(item); }).filter(Boolean) };
}
...
function verifyInitialRoles_() {
  INITIAL_ROLES_.forEach(function(role) {
    ...
    if (role === 'PROPIETARIO' && (permissions.indexOf('*') === -1 || normalizeCode_(row.Protegido) !== 'SI')) throw appError_('ROLE_CONFIGURATION_INVALID', 'El rol PROPIETARIO debe conservar permiso total y protección.', 503);
  });
}
function verifyInitialBranches_() {
  Object.keys(INITIAL_BRANCHES_).forEach(function(branchId) {
    ...
    if (!row || String(row.Nombre || '').trim() !== INITIAL_BRANCHES_[branchId] || normalizeCode_(row.Estado) !== 'ACTIVA') throw appError_('BRANCH_CONFIGURATION_INVALID', 'La sede ' + branchId + ' no coincide con la configuración inicial.', 503);
  });
}
function verifyOwner_() {
  var owners = listRows_('Usuarios').filter(function(row) { return normalizeCode_(row.Rol) === 'PROPIETARIO' && normalizeCode_(row.Estado) === 'ACTIVO'; });
...
function verificarBaseCero() {
  ...
  if (String(spreadsheet.getName() || '').trim() !== MADERARTE_APP.SPREADSHEET_NAME) throw appError_('SPREADSHEET_NAME_MISMATCH', 'La propiedad SPREADSHEET_ID no apunta a la base oficial de Maderarte.', 503);
  verifySchema_();
  ...
  if (normalizeCode_(getConfigValue_('MODO_OPERACION', '')) !== 'PREPARACION') throw appError_('OPERATION_MODE_INVALID', 'MODO_OPERACION debe permanecer en PREPARACION.', 503);
  verifyInitialRoles_(); verifyInitialBranches_();
  var ownerCount = verifyOwner_(), documentsRoot = getDocumentsRoot_();
  if (String(documentsRoot.getName() || '').trim() !== '02_DOCUMENTOS_CLIENTES') throw appError_('DRIVE_ROOT_MISMATCH', 'DRIVE_DOCUMENTS_ROOT_ID no apunta a 02_DOCUMENTOS_CLIENTES.', 503);
  var counts = verifyCommercialBaseZero_();
  var result = { ok: true, version: MADERARTE_APP.VERSION, spreadsheet: MADERARTE_APP.SPREADSHEET_NAME, sheetsVerified: Object.keys(REQUIRED_HEADERS).length, ownerCount: ownerCount, branches: Object.keys(INITIAL_BRANCHES_), commercialWrites: false, mode: 'PREPARACION', commercialCounts: counts, documentsRoot: '02_DOCUMENTOS_CLIENTES' };
