...
function ajEvents_(number) {
  return listRows_('Anulaciones').filter(function(r){return r.Tipo_Entidad==='AJUSTE_OP'&&r.Estado==='CONFIRMADA';}).map(function(r){
    ...
    if(!e||e.contract!==1||!['DESISTIR','RETORNAR','TRANSFERIR','DEVOLVER'].includes(e.type)||!Number.isSafeInteger(e.amount)||e.amount<0)throw appError_('ADJUSTMENT_INTEGRITY','Hay un ajuste que requiere conciliación.',409);
...
function ajCancellationVerified_(item,events){
  if(!Number(item.Cantidad_Desistida||0))return false;
  try{return ajItemAdjustment_(item,events).cancelled===Number(item.Cantidad_Desistida);}catch(e){return false;}
}
function ajPosition_(row,events){
  ...
  events.forEach(function(e){
    if(['DESISTIR','RETORNAR'].includes(e.type)){if(e.source!==row.Numero_OP||!Array.isArray(e.items))ajFail_();reduction+=e.amount;e.items.forEach(function(i){if(e.type==='DESISTIR')cancelled[i.itemId]=(cancelled[i.itemId]||0)+i.quantity;});}
...
function ajPayload_(p){
  ...
  if(['DESISTIR','RETORNAR'].includes(result.type)){
  ...
  if(result.type==='RETORNAR'){if(p.physicalCheck!==true||p.destination!=='EXHIBICION')orderInputError_('physicalCheck','Confirma que el mueble ya regresó al almacén para exhibición.');result.physicalCheck=true;result.destination='EXHIBICION';}
  else if(p.physicalCheck!==undefined||p.destination!==undefined)orderInputError_('physicalCheck','La recepción solo corresponde a una devolución física.');
...
function ajPlan_(p,s){
  ...
  if(p.type==='DESISTIR'){
    ...
    p.items.forEach(function(selection){var i=mdUnique_(source,'Item_ID',selection.itemId);if(!i||selection.quantity>Number(i.Cantidad_Pendiente))throw appError_('ADJUSTMENT_QUANTITY','Solo puedes retirar cantidades pendientes de esta OP.',409);
      ...
      var n=Number(i.Cantidad),c=Number(i.Cantidad_Desistida||0)+ajItemAdjustment_(i,ajEvents_(p.number)).returned,net=Number(i.Valor_Neto),value=Number(BigInt(net)*BigInt(c+selection.quantity)/BigInt(n)-BigInt(net)*BigInt(c)/BigInt(n));
...
function ajConfirm_(payload,context){
  ...
  try{var s=ajSession_(context,true),hash=sha256_(JSON.stringify(p)),replay=ajReplay_(id,s,hash);if(replay){clearConfirmedOrderFence_();return {saved:true,result:replay};}assertNoUnresolvedOrderFence_();
    ...
    if(p.type==='RETORNAR')event.returnReceipt={physicalCheck:true,destination:p.destination,receivedAt:stamp,receivedBy:uid};
    ...
    plan.items.forEach(function(i){if(p.type==='RETORNAR'){requests=requests.concat(orderUpdateRequests_('Orden_Items',i.row._row,{Version:Number(i.row.Version)+1,Actualizado_En:stamp}));return;}var row=i.row,c=Number(row.Cantidad_Desistida||0)+i.quantity,pending=Number(row.Cantidad_Pendiente)-i.quantity;requests=requests.concat(orderUpdateRequests_('Orden_Items',row._row,{Cantidad_Desistida:c,Cantidad_Pendiente:pending,Estado_Item:pending===0?'DESISTIDO':row.Estado_Item,Version:Number(row.Version)+1,Actualizado_En:stamp}));});
...
function ajReceiptHistory_(payment,row){
  ...
  ajEvents_(row.Numero_OP).filter(function(e){return e.date<=date;}).forEach(function(e){var incoming=e.target===row.Numero_OP;history.push({number:e.id,date:e.date,method:['DESISTIR','RETORNAR'].includes(e.type)?'AJUSTE DE PEDIDO':e.type==='DEVOLVER'?'DEVOLUCIÓN':incoming?'SALDO RECIBIDO':'SALDO TRASLADADO',amount:['DESISTIR','RETORNAR'].includes(e.type)?0:incoming?e.amount:-e.amount,balance:(incoming?e.targetAfter:e.after).balance});});
