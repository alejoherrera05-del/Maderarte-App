// Order creation contract v1 with optional documentary v2 manifests.
// No work on module load. Production retains three closed activation gates.
...
function orderInputError_(field, message) { throw appError_('ORDER_INPUT_INVALID', message, 400, { field: field }); }
function orderObject_(value, allowed, field) {
  ...
  Object.keys(value).forEach(function(key) { if (allowed.indexOf(key) === -1) orderInputError_(field + '.' + key, 'El pedido contiene un campo no admitido.'); });
  return value;
}
function orderText_(value, field, max, required) {
  ...
  var text = value.trim();
  if ((required && !text) || text.length > max || /[\u0000-\u0008\u000b\u000c\u000e-\u001f]/.test(text)) orderInputError_(field, 'Completa o corrige este dato del pedido.');
  return text;
}
...
function normalizeOrderCreation_(payload) {
  ...
  if (payload.schemaVersion !== ORDER_CREATION_CONTRACT_ && !(payload.schemaVersion === 2 && typeof normalizeOrderPhotoRefs_ === 'function')) throw appError_('ORDER_CONTRACT_MISMATCH', 'Actualiza la aplicación antes de guardar.', 409);
  ...
  var client = {};
  [['document', 40], ['name', 250], ['phone', 40], ['alternatePhone', 40], ['email', 254], ['address', 500], ['city', 120]].forEach(function(spec) { client[spec[0]] = orderText_(rawClient[spec[0]], 'client.' + spec[0], spec[1], spec[0] !== 'alternatePhone'); });
  ...
  var items = payload.items.map(function(raw, index) {
    ...
    if (payload.schemaVersion !== 2 && raw.photos !== undefined && (!Array.isArray(raw.photos) || raw.photos.length)) throw appError_('ORDER_PHOTOS_NOT_READY', 'El guardado de fotografías aún no está habilitado. Conserva el borrador con todas sus referencias.', 409, { field: path + '.photos' });
    ...
    return { clientLineId: id, position: index + 1,
      description: orderText_(raw.description, path + '.description', 500, true), category: orderText_(raw.category, path + '.category', 40, false),
      quantity: quantity, unitValue: unitValue, subtotal: gross, fabric: orderText_(raw.fabric, path + '.fabric', 500, false), wood: orderText_(raw.wood, path + '.wood', 500, false),
      specifications: orderText_(raw.specifications, path + '.specifications', 8000, false), agreement: agreement, fulfillment: fulfillment,
      ...(payload.schemaVersion === 2 ? { photos: normalizeOrderPhotoRefs_(raw.photos, path + '.photos') } : {}) };
  });
  ...
  var parts = items.map(function(item, index) { var numerator = BigInt(item.subtotal) * BigInt(discount); return { index: index, discount: Number(numerator / BigInt(subtotal)), remainder: numerator % BigInt(subtotal) }; });
  ...
  var payments = payload.payments.map(function(raw, index) {
    ...
    paymentIds.add(id);
    var amount = orderInteger_(raw.amount, path + '.amount', 1); paid = orderInteger_(paid + amount, 'paid', 1);
    return { clientPaymentId: id, method: orderEnum_(raw.method, ['EFECTIVO', 'TRANSFERENCIA', 'TARJETA', 'ADDI'], path + '.method'), amount: amount,
      internalNote: orderText_(raw.internalNote, path + '.internalNote', 2000, false), reference: orderText_(raw.reference, path + '.reference', 250, false), comment: orderText_(raw.comment, path + '.comment', 1000, false) };
  });
  ...
  return { schemaVersion: payload.schemaVersion, branch: branch, client: client, items: items, payments: payments,
    discount: discount, notes: orderText_(payload.notes, 'notes', 10000, false), noPayment: payload.noPayment, subtotal: subtotal, total: total, paid: paid, balance: total - paid };
}
...
function orderCreationSchemaReady_() {
  if (['2', '3'].indexOf(optionalProperty_('ORDER_SCHEMA_VERSION', '1')) < 0) throw appError_('ORDER_SCHEMA_NOT_READY', 'La base aún no está preparada para guardar pedidos.', 503);
  verifySchema_();
...
function orderCell_(value) {
  ...
  // Never interpret customer input as a spreadsheet formula.
  return { userEnteredValue: { stringValue: String(value) } };
}
function orderAppendRequest_(name, objects) {
  var sheet = getSheet_(name), headers = getHeaders_(sheet);
  objects.forEach(function(object) { Object.keys(object).forEach(function(key) { if (headers.indexOf(key) === -1) throw appError_('SHEET_SCHEMA_MISMATCH', 'Falta una columna para guardar íntegramente el pedido.', 503); }); });
  return { appendCells: { sheetId: sheet.getSheetId(), rows: objects.map(function(object) { return { values: headers.map(function(header) { return orderCell_(object[header]); }) }; }), fields: 'userEnteredValue' } };
}
...
function orderAtomicBatch_(requests) {
  ...
  var response = UrlFetchApp.fetch('https://sheets.googleapis.com/v4/spreadsheets/' + encodeURIComponent(requiredProperty_('SPREADSHEET_ID')) + ':batchUpdate', { method: 'post', contentType: 'application/json', headers: { Authorization: 'Bearer ' + ScriptApp.getOAuthToken() }, payload: body, muteHttpExceptions: true });
...
function orderNextNumbers_(branchRow, type, count) {
  if (!count) return [];
  var field = type === 'OP' ? 'OP' : 'Recibo', prefix = String(branchRow['Prefijo_' + field] || '').trim().replace(/-+$/, ''), next = branchRow['Siguiente_' + field];
  if (typeof next === 'string' && /^\d+$/.test(next)) next = Number(next);
  if (!Number.isSafeInteger(next) || next < 1 || !Number.isSafeInteger(next + count) || !/^[A-Z0-9]+(?:-[A-Z0-9]+)*$/.test(prefix) || prefix.indexOf(branchRow.Sede_ID + '-') !== 0) throw appError_('NUMBERING_NOT_READY', 'Revisa los prefijos y consecutivos de esta sede.', 503);
  var registered = listRows_('Registro_Numeros'), existing = listRows_(type === 'OP' ? 'Ordenes_Pedido' : 'Abonos');
  return Array.from({ length: count }, function(_, index) {
    var number = prefix + '-' + String(next + index).padStart(4, '0');
    if (registered.some(function(row) { return row.Numero === number; }) || existing.some(function(row) { return row[type === 'OP' ? 'Numero_OP' : 'Numero_Recibo'] === number; })) throw appError_('NUMBER_ALREADY_USED', 'El consecutivo ya fue utilizado. Requiere revisión, no se reutilizará.', 409);
    return number;
  });
}
function buildOrderCreationBatch_(draft, requestId, session, branchRow, clientRow, fingerprint) {
  var stamp = now_().toISOString();
  var number = orderNextNumbers_(branchRow, 'OP', 1)[0], receiptNumbers = orderNextNumbers_(branchRow, 'RECIBO', draft.payments.length);
  ...
  var client = draft.client, user = session.profile.uid;
  ...
  var result = { number: number, branch: draft.branch, requestId: requestId, revision: 1, total: draft.total, paid: draft.paid, balance: draft.balance,
    documentStatus: 'PENDIENTE', items: items.map(function(item) { return { clientLineId: item.clientLineId, id: item.id }; }), receipts: draft.payments.map(function(payment, index) { return { number: receiptNumbers[index], amount: payment.amount, method: payment.method }; }) };
  var requests = [];
  if (!clientRow) requests.push(orderAppendRequest_('Clientes', [{ Cedula_NIT: client.document, Nombre_Completo: client.name, Telefono: client.phone,
    Telefono_Alterno: client.alternatePhone, Email: client.email, Direccion: client.address, Ciudad: client.city, Sede_Origen: draft.branch, Fecha_Registro: stamp, Ultima_Compra: stamp, Estado: 'ACTIVO' }]));
  ...
  if (draft.payments.length) requests.push(orderAppendRequest_('Abonos', draft.payments.map(function(payment, index) {
    ...
    return { Numero_Recibo: receiptNumbers[index], Numero_OP: number, Sede: draft.branch, Cedula_NIT: client.document, Nombre_Cliente: client.name,
      ...
      Nota_Interna: payment.internalNote, Saldo_Anterior: previous, Saldo_Nuevo: balance, Registrado_Por: user, Fecha_Registro: stamp, Estado_Registro: 'ACTIVO', Afecta_Saldo: 'SI', Request_ID: requestId }; })));
  requests.push(orderAppendRequest_('Registro_Numeros', [number].concat(receiptNumbers).map(function(value, index) { return {
    Registro_ID: requestId + '-N' + index, Sede: draft.branch, Tipo_Documento: index ? 'RECIBO' : 'OP', Numero: value, Estado: 'CONFIRMADO', Entidad_ID: index ? value : number, Reservado_En: stamp, Confirmado_En: stamp, Usuario: user, Request_ID: requestId }; })));
  ...
  requests.push(orderAppendRequest_('Auditoria', [{ ID: requestId + '-AUD', Fecha: stamp, Usuario: user, Rol: session.profile.role || '',
    ...
    Estado: 'CONFIRMADA', Request_ID: requestId, Antes_JSON: '{}', Despues_JSON: JSON.stringify(result), Reversible: 'NO', Motivo_No_Reversible: 'Los cambios requieren movimientos posteriores; nunca borrar la orden.' }]));
  requests.push(orderAppendRequest_('Idempotencia', [{ Request_ID: requestId, Fecha: stamp, Tipo_Operacion: 'ORDEN_CREAR', Entidad: 'ORDEN', Entidad_ID: number,
    Estado: 'CONFIRMADA', Resultado_JSON: JSON.stringify({ fingerprint: fingerprint, result: result }), Usuario: user, Dispositivo_ID: session.sessionRow && session.sessionRow.Dispositivo_ID || '' }]));
  return { requests: requests, result: result };
}
function createOrder_(payload, context) {
  if (!(typeof orderOperationsAllowed_ === 'function' ? orderOperationsAllowed_() : (MADERARTE_APP.COMMERCIAL_WRITES && getConfigValue_('MODO_OPERACION', 'PREPARACION') === 'OPERACION' && optionalProperty_('ORDER_SAVE_ENABLED', 'NO') === 'SI'))) throw appError_('COMMERCIAL_WRITES_DISABLED', 'El guardado comercial todavía no está habilitado.', 403);
  var draft = normalizeOrderCreation_(payload), requestId = orderRequestId_(context.requestId), lock = LockService.getScriptLock();
  if (!lock.tryLock(5000)) throw appError_('ORDER_SAVE_BUSY', 'Hay otro guardado en curso. Reintenta con el mismo pedido.', 503);
  try {
    if (!(typeof orderOperationsAllowed_ === 'function' ? orderOperationsAllowed_() : (MADERARTE_APP.COMMERCIAL_WRITES && getConfigValue_('MODO_OPERACION', 'PREPARACION') === 'OPERACION' && optionalProperty_('ORDER_SAVE_ENABLED', 'NO') === 'SI'))) throw appError_('COMMERCIAL_WRITES_DISABLED', 'El guardado fue deshabilitado. Conserva el borrador.', 403);
    // Identity is checked AFTER locking, always against the original auth base.
    var session = (typeof validateOrderSession_ === 'function' ? validateOrderSession_ : validateSessionToken_)(context.sessionToken, false);
    orderCreationAllowed_(session, draft.branch);
    if (draft.payments.length) requirePermission_(session, 'abonos.create');
    if (getSpreadsheet_().getName() !== (typeof orderExpectedSpreadsheetName_ === 'function' ? orderExpectedSpreadsheetName_() : MADERARTE_APP.SPREADSHEET_NAME)) throw appError_('SPREADSHEET_NAME_MISMATCH', 'La base configurada no corresponde a Maderarte.', 503);
    ...
    if (replay) { clearConfirmedOrderFence_(); return { saved: true, replayed: true, order: replay }; }
    assertNoUnresolvedOrderFence_();
    if (typeof orderDocumentFenceCheck_ === 'function') orderDocumentFenceCheck_();
    if (payload.schemaVersion === 2 && optionalProperty_('ORDER_SCHEMA_VERSION', '1') !== '3') throw appError_('DOCUMENT_SCHEMA_NOT_READY', 'Falta preparar los documentos.', 503);
    ...
    // Admission fence remains until the atomic batch's result is witnessed.
    reserveOrderFence_(requestId, session.profile.uid, fingerprint);
    try { orderAtomicBatch_(batch.requests); }
    catch (error) { throw appError_('ORDER_SAVE_UNCERTAIN', 'No se confirmó la respuesta del guardado. Consulta el resultado con el mismo identificador antes de iniciar otro pedido.', 503, { requestId: requestId }); }
...
function orderCreationStatus_(payload, context) {
  ...
  var requestId = orderRequestId_(payload.requestId), lock = LockService.getScriptLock();
...
function orderCreationCapabilities_(session) {
  ...
  var ready = typeof orderOperationsAllowed_ === 'function' && optionalProperty_('ORDER_SCHEMA_VERSION', '1') === '3';
  return { contractVersion: ready ? 2 : ORDER_CREATION_CONTRACT_, enabled: Boolean(ready && orderOperationsAllowed_()), reason: ready && orderOperationsAllowed_() ? 'DISPONIBLE' : 'PREPARACION',
    persistenceImplemented: true, photosReady: Boolean(ready), documentsReady: Boolean(ready), trial: typeof orderIsTrial_ === 'function' && orderIsTrial_(), documentRevision: typeof ORDER_DOCUMENT_REVISION_ === 'undefined' ? '' : ORDER_DOCUMENT_REVISION_ };
}
// Owner-run installation step. Never imports or changes commercial records.
function prepararEsquemaGuardadoOrdenes() {
  if (MADERARTE_APP.COMMERCIAL_WRITES || getConfigValue_('MODO_OPERACION', 'PREPARACION') !== 'PREPARACION') throw appError_('SCHEMA_SETUP_NOT_ALLOWED', 'Prepara el esquema solo con la operación comercial deshabilitada.', 403);
  var lock = LockService.getScriptLock();
  if (!lock.tryLock(5000)) throw appError_('ORDER_SAVE_BUSY', 'Hay otra operación en curso.', 503);
  try {
    if (getSpreadsheet_().getName() !== (typeof orderExpectedSpreadsheetName_ === 'function' ? orderExpectedSpreadsheetName_() : MADERARTE_APP.SPREADSHEET_NAME)) throw appError_('SPREADSHEET_NAME_MISMATCH', 'Base incorrecta.', 503);
    verifyCommercialBaseZero_();
    var requests = [];
    Object.keys(REQUIRED_HEADERS).forEach(function(name) {
      var sheet = getSheet_(name), actual = getHeaders_(sheet), base = REQUIRED_HEADERS[name], extra = ORDER_CREATION_EXTRA_HEADERS_[name] || [], expanded = base.concat(extra);
