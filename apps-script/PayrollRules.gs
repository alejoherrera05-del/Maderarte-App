...
function payrollDate(v){if(!/^\d{4}-\d{2}-\d{2}$/.test(v||'')||!Number.isFinite(new Date(v+'T12:00:00Z').getTime())||new Date(v+'T12:00:00Z').toISOString().slice(0,10)!==v)throw Error('Revisa la fecha.');return v;}
...
function payrollBenefitDefaults(to,employee,configuredRates=PAYROLL_RATES){
...
function payrollBenefitPeriods(from,to){
