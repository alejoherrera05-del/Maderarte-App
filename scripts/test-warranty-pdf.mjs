...
const req=(cookie=true,kind='')=>new Request('https://app.example.com/api/maderarte',{method:'POST',headers:{Origin:'https://app.example.com','Content-Type':'application/json',...(cookie?{Cookie:'__Host-maderarte_session=qa-session'}:{})},body:JSON.stringify({action:'GARANTIA_PDF',payload:{id,...(kind?{kind}:{}),document:{number:'INJECTED'},url:'https://evil.example'}})});
...
try{
 ...
 assert.equal((await handleRequest(req(true,'ENTREGA'),env)).status,503);
 plan.type='ENTREGA';globalThis.fetch=async(_u,o)=>{assert.deepEqual(JSON.parse(o.body).payload,{id,kind:'ENTREGA'});return Response.json({status:'success',data:plan});};
 const delivered=await handleRequest(req(true,'ENTREGA'),env);assert.equal(delivered.status,200);assert.equal((await delivered.json()).data.name,'Entrega-garantia-'+id+'.pdf');
