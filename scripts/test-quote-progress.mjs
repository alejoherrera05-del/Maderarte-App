...
import { finishQuoteDocuments } from '../public/js/core/quote-media.js';
...
for (const photosCount of [0, 1, 2]) {
  ...
  const files = () => [...media.map((p,i)=>({id:'slot'+i,type:'FOTO',...p,itemId:p.clientLineId,ready:uploaded.has('slot'+i),url:uploaded.has('slot'+i)?'https://drive.google.com/file/d/photo'+i+'/view':''})),{id:'pdf',type:'COTIZACION',ready:finalized,url:finalized?'https://drive.google.com/file/d/pdf/view':''}];
  const request = async(action,payload) => {
    if(action==='COTIZACION_DOCUMENTOS_ESTADO'){reads++;return {data:{number,complete:finalized,files:files()}};}
    if(action==='COTIZACION_FOTO_GUARDAR'){
  ...
  const result=await finishQuoteDocuments(number,media,request,()=>{},e=>events.push(e));
  ...
  const resumed=[];await finishQuoteDocuments(number,[],request,()=>{},e=>resumed.push(e));
...
for (const fault of ['upload','finalize','readback','wrong-photo']) {
  ...
  const request=async(action,payload)=>{
    if(action==='COTIZACION_DOCUMENTOS_ESTADO'){
      reads++;if(fault==='readback'&&reads===2)throw Error('lost status');
      return {data:{number,complete:finalized,files:[{id:'photo',itemId:'1',type:'FOTO',photoId:'p',sha256:'a',ready:false,url:''},{id:'pdf',type:'COTIZACION',ready:finalized,url:finalized?'url':''}]}};
    }
    if(action==='COTIZACION_FOTO_GUARDAR'){
  ...
  await assert.rejects(()=>finishQuoteDocuments(number,[{clientLineId:'1',photoId:'p',sha256:'a',base64:'x'}],request,()=>{},e=>events.push(e)));checks++;
