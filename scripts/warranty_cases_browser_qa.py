...
try:
 ...
 with sync_playwright() as p:
  ...
  for width in [1440,390,320]:
   ...
   def route(r):
    ...
    if a=='AUTH_SESSION_VALIDATE':data=SESSION
     ...
     if v['operation'] in ['receive','home','report'] and not v['id']:
      ...
      c=dict(v,id='QA-REPORT' if v['operation']=='report' else 'QA-HOME' if v['operation']=='home' else 'QA-CASE',client=ORDER['client'],description=ITEM['description'],branch='MP',revision=1,events=[]);cases.append(c)
   ...
   page.locator('#wc-pdf').click();expect(page.locator('#wc-pdf-link a')).to_have_attribute('href',__import__('re').compile('^blob:'));assert len(writes)==5;assert not errors,errors
   ...
   page.locator('#ag-back').click();expect(page.locator('#wc-title')).to_have_text('Garantía');expect(page.locator('#wc-visits')).to_contain_text('2099-12-20');page.locator('#wc-dialog').evaluate('e=>{e.getAnimations().forEach(a=>a.finish());e.scrollTop=0}');page.screenshot(path=str(OUT/f'connected-case-{width}.png'),full_page=True)
   page.locator('#wc-visits a.wc-pick').click();expect(page.locator('#ag-detail')).to_be_visible();page.locator('#ag-back').click();expect(page.locator('#wc-title')).to_have_text('Garantía');page.locator('#wc-receive-case').click();expect(page.locator('#wc-piece')).to_have_value('Silla del comedor');expect(page.locator('#wc-issue')).to_have_value('Respaldo suelto');page.locator('#wc-condition').fill('Una silla sin accesorios');page.locator('[name=physicalCheck]').check();page.locator('#wc-form [type=submit]').click();expect(page.locator('#wc-title')).to_have_text('Garantía');assert writes[-1]['id']=='QA-REPORT';assert len(cases)==3;assert not errors,errors
   page.locator('#wc-dialog').evaluate('e=>{e.getAnimations().forEach(a=>a.finish());e.scrollTop=0}');page.screenshot(path=str(OUT/f'connected-reception-{width}.png'),full_page=True);assert page.locator('body').evaluate('e=>e.scrollWidth<=innerWidth');ctx.close()
