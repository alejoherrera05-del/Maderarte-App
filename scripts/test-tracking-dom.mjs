...
rows[0].URL_PDF_Cotizacion = 'https://drive.google.com/file/d/TEST_document-01/view?resourcekey=TEST_key';
...
try {
  ...
  const pdfButton = document.querySelector('[data-pdf-url]');
  pdfButton.focus();
  pdfButton.click();
  assert.equal(document.getElementById('tracking-pdf-frame').src, 'https://drive.google.com/file/d/TEST_document-01/preview?resourcekey=TEST_key');
  assert.equal(document.getElementById('tracking-pdf-open').href, rows[0].URL_PDF_Cotizacion);
  assert.equal(document.getElementById('tracking-app').inert, true);
  document.getElementById('tracking-pdf-close').click();
  assert.equal(document.getElementById('tracking-app').inert, false);
  assert.equal(document.activeElement, pdfButton);
  assert.equal(document.getElementById('tracking-pdf-frame').src, 'about:blank');
