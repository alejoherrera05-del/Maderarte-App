...
import { createQuoteSave, quoteCapabilitiesReady } from '../public/js/core/quote-save.js';
...
function harness() {
  ...
  const request = async (action, payload, options) => {
    ...
    if (action === 'COTIZACION_CAPACIDADES') return { data: caps };
    if (action === 'COTIZACION_CREAR') {
      if (delay) await delay;
      if (createMode === 'before-timeout') throw new Error('timeout before commit');
      if (createMode === 'input') throw Object.assign(new Error('Corrige el campo'), { code: 'QUOTE_INPUT_INVALID', requestId: options.requestId });
      const order = { requestId: options.requestId, number: 'MP-QA-COT-0001', branch: 'MP' };
      orders.set(options.requestId, order);
      if (createMode === 'after-timeout') throw new Error('lost response');
      if (createMode === 'wrong-id') return { data: { saved: true, quote: { ...order, requestId: 'WRONG-REQUEST-00000001' } } };
      return { data: { saved: true, quote: order } };
    }
    if (action === 'COTIZACION_CREACION_ESTADO') {
      if (statusMode === 'offline') throw new Error('offline');
      if (orders.has(payload.requestId)) return { data: { saved: true, quote: orders.get(payload.requestId) } };
    ...
    if (action === 'COTIZACION_DOCUMENTOS_ESTADO') return { data: { number: payload.number, complete: true, files: [{type:'COTIZACION',ready:true,url:'https://example.invalid/pdf'}] } };
    throw new Error(`Unexpected action ${action}`);
  };
  const make = (temp = temporary, overrides = {}) => createQuoteSave({ uid: 'qa-save', request, durable, temporary: temp, locks, crypto: webcrypto, activeUid: () => uid, ...overrides });
...
for (const caps of [null, {}, { ...ready, enabled: 'true' }, { ...ready, contractVersion: 2 }, { ...ready, photosReady: false }, { ...ready, documentsReady: false }]) equal(quoteCapabilitiesReady(caps), false);
equal(quoteCapabilitiesReady(ready), true);
{
  ...
  equal(h.calls.filter(x => x.action === 'COTIZACION_CREAR').length, 0); equal(h.durable.getItem(m.key), null);
}
{
  ...
  for (let i = 0; i < 100 && !h.calls.some(x => x.action === 'COTIZACION_CREAR'); i++) await new Promise(resolve => setTimeout(resolve, 2));
  await m.save(body()); equal(h.calls.filter(x => x.action === 'COTIZACION_CREAR').length, 1, 'double click');
  const second = h.make(store()); equal((await second.save(body())).phase, 'other-tab');
  equal(h.calls.filter(x => x.action === 'COTIZACION_CREAR').length, 1, 'cross-tab lock');
  release(); const result = await saving;
  equal(result.phase, 'confirmed'); equal(result.ownsDraft, true);
  equal(h.calls.find(x => x.action === 'COTIZACION_CREAR').payload.payments[0].amount, 10, 'immutable input');
  ...
  await m.save(body()); equal(h.calls.filter(x => x.action === 'COTIZACION_CREAR').length, 1);
...
}
{
  ...
  equal(h.calls.filter(x => x.action === 'COTIZACION_CREAR').length, 1);
...
}
{
  ...
  equal(h.calls.filter(x => x.action === 'COTIZACION_CREAR').length, 1);
  const journal = JSON.parse(raw); h.orders.set(journal.requestId, { requestId: journal.requestId, number: 'MP-QA-COT-0001', branch: 'MP' });
...
}
{
  ...
  equal((await m.retry()).phase, 'confirmed'); const attempts = h.calls.filter(x => x.action === 'COTIZACION_CREAR');
...
}
{
  ...
  await m.retry(); equal(h.calls.filter(x => x.action === 'COTIZACION_CREAR').length, 1);
}
{
...
{
  const h = harness(), m = h.make(); h.setMode('before-timeout'); await m.save(body());
  const raw = h.durable.getItem(m.key); h.temporary.removeItem(m.key);
...
for (const failureStore of ['durable', 'temporary']) {
  ...
  equal((await m.save(body())).phase, 'blocked'); equal(h.calls.filter(x => x.action === 'COTIZACION_CREAR').length, 0);
}
{
