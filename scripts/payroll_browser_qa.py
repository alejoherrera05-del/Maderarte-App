...
try:
 ...
 with sync_playwright() as p:
  ...
  for width in [1440,390,320]:
   ...
   expect(page.locator('.np-summary-date')).to_contain_text('Vencido desde')
   expect(page.locator('.np-summary-date')).to_contain_text('15')
   ...
   expect(page.locator('.np-operation-title')).to_have_text('Comprobante listo')
   page.screenshot(path=str(out/f'feedback-listo-{width}.png'))
   page.evaluate("""async()=>{const m=await import('/js/core/payroll-feedback.js');m.hidePayrollFeedback(document.querySelector('#payroll-app'));}""")
   ...
   expect(page.get_by_text('Prima de servicios',exact=True)).to_be_visible()
   expect(page.get_by_text('Cesantías',exact=True)).to_be_visible()
   expect(page.get_by_text('Intereses a las cesantías',exact=True)).to_be_visible()
   expect(page.get_by_text('Vacaciones pendientes',exact=True)).to_be_visible()
