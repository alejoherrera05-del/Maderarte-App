...
CLIENT = {
    'document': '0000000001', 'name': 'Cliente de muestra',
    'phone': '0000000011', 'alternatePhone': '0000000022',
    'email': 'qa@example.invalid', 'address': 'Dirección de muestra', 'city': 'Popayán (prueba)'
}
ITEMS = [
...
def fill_document(page, kind, many=False):
    ...
    # Keep synthetic, deterministic identifiers only in the local preview.
    ...
    with sync_playwright() as p:
        ...
        for width in (1440, 390, 320):
            ...
            for kind in ('quote', 'order'):
                ...
                assert '14.400.000' in text and all(t[0] in text for t in ITEMS)
                if kind == 'quote':
                    assert 'total cotizado' in folded and 'condiciones comerciales' in folded
                    assert 'Pagado hoy' not in text and 'Saldo pendiente' not in text and 'Abono indicado' not in text
                    assert page.locator('.order-finance').count() == 0
                    assert 'fabricación' in folded
                else:
                    assert 'Saldo pendiente' in text and '12.700.000' in text
                ...
                if width == 1440:
                    ...
                    assert '14.400.000' in printed and 'sin validez comercial' in printed
        ...
        # Long quotes use the same scoped styles when paginating: no clipped last items or duplicate closing.
        context = browser.new_context(viewport={'width': 1440, 'height': 1050})
        page = context.new_page()
        fill_document(page, 'quote', many=True)
        for i in range(18): assert f'Mueble de control {i+1:02d}' in page.locator('#quote-preview-content').inner_text()
        assert page.locator('.quote-editorial-investment').count() == 1
        pages = page.locator('.quote-preview-page').count()
        assert pages > 1
        assert page.locator('.order-finance').count() == 0
        # Confirm that the measured preview fits its fixed A4 boxes.
