...
const imported=sandboxRuntime();let importSequence=0;const importCall=(a,p)=>imported.c.routeAction_(a,p,{...imported.ctx,requestId:'IMPORT-TEST-'+(++importSequence),session:imported.c.validateSessionToken_(imported.ctx.sessionToken,false)});
const pendingWorker=importCall('NOMINA_TRABAJADOR_GUARDAR',{name:'Ficha sin fecha de muestra',document:'SYNTHETIC-PENDING',start:'',role:'',branch:'',contract:'',salary:0,transport:true});
assert.equal(pendingWorker.start,'');assert.equal(pendingWorker.branch,'');assert.equal(pendingWorker.contract,'');
const pendingRun=importCall('NOMINA_QUINCENA',{month:'2026-09',half:'1'});assert.equal(pendingRun.items[0].days.periodDays,15);
assert.throws(()=>payrollCalculate({type:'PRIMA',from:'2026-07-01',to:'2026-12-31',benefitBase:2000000,reviewed:true},pendingWorker),/fecha real/);
assert.throws(()=>payrollCalculate({type:'LIQUIDACION',from:'2026-07-01',to:'2026-12-31',benefitBase:2000000,reviewed:true},pendingWorker),/fecha real/);
assert.throws(()=>importCall('NOMINA_TRABAJADOR_GUARDAR',{...pendingWorker,id:undefined}),/ya tiene/);
console.log('Pending worker details: no invented employment dates, ordinary fortnight allowed, benefits blocked and duplicate identity protected.');
