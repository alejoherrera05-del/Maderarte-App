...
assert.equal(settlement.lines.find(l=>l.label==='Intereses a las cesantías · 2026').amount,240000);
const autoPrima=payrollCalculate({type:'PRIMA',from:'2026-01-01',to:'2026-12-31',benefitBase:2000000,reviewed:true},employee);assert.equal(autoPrima.lines.find(l=>l.label.startsWith('Prima de servicios')).amount,1000000);assert.deepEqual(autoPrima.coverage.filter(x=>x.concept==='PRIMA').map(x=>x.from),['2026-01-01','2026-07-01']);
