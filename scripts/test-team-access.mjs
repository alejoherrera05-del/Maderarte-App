...
const tables=new Proxy({},{get:(_,k)=>f.production().tables[k],set:(_,k,v)=>(f.production().tables[k]=v,true)});
for(const name of ['Invitaciones'])tables[name]||={id:900,headers:[...c.REQUIRED_HEADERS[name]],rows:[]};
tables.Roles.rows.push({Rol:'VENDEDOR',Activo:'SI',Permisos_JSON:'["app.access","ordenes.read","ordenes.create","clientes.read","clientes.create"]'},{Rol:'ADMINISTRADOR',Activo:'SI',Permisos_JSON:'["app.access","users.manage","ordenes.read"]'},{Rol:'CONSULTA',Activo:'NO',Permisos_JSON:'["app.access"]'});
...
const admin={profile:{uid:'admin',role:'ADMINISTRADOR',branches:['MP']},permissions:['users.manage','app.access','ordenes.read','recaudos.read','recaudos.receive']};
