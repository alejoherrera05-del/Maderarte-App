...
guardPage({
  ...
  async render({ session, content }) {
    ...
    content.innerHTML = `<section class="page"><div class="page-header"><div class="page-heading"><h1>${escapeHtml(session.profile.name || 'Mi cuenta')}</h1><p>Tu rol y tus permisos corresponden exclusivamente a Maderarte.</p></div><div class="page-actions"><button class="button secondary" id="logout-button" type="button">Cerrar sesión</button></div></div>
