...
function renderItems(){
  ...
  for(const [index,item] of account.position.items.entries()){
    ...
    row.innerHTML=`<input type="checkbox" id="rm-select-${index}" ${available?'':'disabled'}><label for="rm-select-${index}">${esc(item.description)}<small class="rm-item-status">${item.pending===0?'✓ Despacho completo':`${item.delivered} despachadas · ${item.pending} pendientes`}</small>${item.blocked&&item.pending?`<small>${esc(item.blocked)}</small>`:''}</label><label class="rm-quantity-label" for="rm-qty-${index}">Salen hoy<input id="rm-qty-${index}" type="number" min="1" max="${item.available??item.pending}" step="1" inputmode="numeric" disabled aria-label="Cantidad a despachar: ${esc(item.description)}"></label>`;
