...
guardPage({
  ...
  subtitle: 'Una fila por OP y acceso directo a su expediente.',
  async render({ content }) {
    content.innerHTML = `<section class="page">
      <div class="page-header"><div class="page-heading"><h1>Órdenes de pedido</h1><p>Consulta por número, cliente, cédula, teléfono, sede o estado.</p></div><div class="page-actions"><span class="status-badge info" id="orders-count">0 órdenes</span></div></div>
      <form class="toolbar" id="orders-filter"><div class="field"><label for="order-search">Buscar</label><input id="order-search" type="search" placeholder="OP, cliente, cédula o teléfono" autocomplete="off"></div><div class="field"><label for="order-branch">Sede</label><select id="order-branch"><option value="">Todas</option><option value="MP">Principal</option><option value="TP">Terraplaza</option></select></div><div class="field"><label for="order-status">Estado</label><select id="order-status"><option value="">Todos</option><option value="CONFIRMADA">Confirmada</option><option value="EN_PROCESO">En proceso</option><option value="COMPLETADA">Completada</option><option value="ANULADA">Anulada</option></select></div><div class="field"><label for="production-status">Producción</label><select id="production-status"><option value="">Todas</option><option value="PENDIENTE">Pendiente</option><option value="EN_PROCESO">En proceso</option><option value="LISTA">Lista</option><option value="COMPLETADA">Completada</option></select></div><button class="button secondary" type="submit">Aplicar</button></form>
