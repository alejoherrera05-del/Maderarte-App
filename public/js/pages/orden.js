...
function renderItems(items) {
  ...
  return `<div class="od-items">${items.map(item => `<article class="od-item" data-order-item="${escapeHtml(item.id)}"><div class="od-item-copy"><strong>${escapeHtml(item.description || 'Producto')}</strong>${item.agreement ? `<span>${escapeHtml(item.fulfillment === 'PARA_SOLICITAR' ? 'Solicitar a fábrica' : item.agreement === 'ENTREGA_HOY' ? 'Entrega inmediata' : 'Separado / entregar después')}</span>` : ''}<span>${escapeHtml([item.reference, item.measures, item.fabricColor, item.woodColor, item.specifications].filter(Boolean).join(' · ') || 'Sin especificaciones adicionales')}</span></div><div class="od-item-amount">${item.delivered > 0 ? `<span class="${item.pending === 0 ? 'od-dispatch-complete' : 'od-dispatch-partial'}">${item.pending === 0 ? '✓ Despacho completo' : escapeHtml(item.delivered + ' despachadas · ' + item.pending + ' pendientes')}</span>` : ''}<strong>${escapeHtml(String(item.quantity ?? 0))}</strong><span>${escapeHtml(item.unit || 'UNIDAD')}</span><small>${escapeHtml(money(item.subtotal))}</small></div></article>`).join('')}</div>`;
}
...
function renderOrder(data, session) {
  ...
  <main class="od-shell">
    ...
    <aside class="od-stack"><section class="od-card"><div class="od-card-head"><h2>Documentos principales</h2></div>${quoteLink}${primaryLinks || '<div class="od-empty">No hay enlaces disponibles todavía.</div>'}</section><section class="od-card"><div class="od-card-head"><h2>Remisiones</h2><span class="od-count">${remissions.length}</span></div>${renderRemissions(remissions)}${hasPermission(session, 'remisiones.read') ? `<a class="od-link" href="${escapeHtml(sandboxLink('/remision.html?op=' + encodeURIComponent(order.number)))}">Preparar despacho · remisiones y pendientes</a>` : ''}</section><section class="od-card"><div class="od-card-head"><h2>Documentos</h2><span class="od-count">${documents.length}</span></div>${renderDocuments(documents)}</section></aside></div>
  </main>`;
}
