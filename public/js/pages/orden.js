...
function renderItems(items) {
  ...
  return `<div class="od-items">${items.map(item => `<article class="od-item"><div class="od-item-copy"><strong>${escapeHtml(item.description || 'Producto')}</strong><span>${escapeHtml([item.fulfillment === 'PARA_SOLICITAR' ? 'Solicitar a fábrica' : item.agreement === 'ENTREGA_HOY' ? 'Entrega inmediata' : item.agreement === 'SEPARADO' ? 'Separado / entregar después' : '', item.reference, item.measures, item.fabricColor, item.woodColor, item.specifications].filter(Boolean).join(' · ') || 'Sin especificaciones adicionales')}</span></div><div class="od-item-amount"><strong>${escapeHtml(String(item.quantity ?? 0))}</strong><span>${escapeHtml(item.unit || 'UNIDAD')}</span><small>${escapeHtml(money(item.subtotal))}</small></div></article>`).join('')}</div>`;
}
...
function renderOrder(data) {
  ...
  <main class="od-shell">${APP_CONFIG.trial ? '<p class="order-trial-banner">ENSAYO AISLADO · Sin validez comercial. No afecta la base original.</p>' : ''}
  ...
  async render({ session }) {
    ...
    try {
      ...
      if (!APP_CONFIG.preview.enabled) {
        const capabilities = await apiRequest('ORDEN_CAPACIDADES', {}).catch(() => ({ data: null }));
        if (capabilities.data?.contractVersion === 2) {
          const panel = document.createElement('section'); panel.className = 'od-card'; root.querySelector('.od-stack').prepend(panel);
          mountOrderDocuments({ number, uid: session.profile.uid, host: panel, auto: response.data.order.documentStatus === 'COMPLETO' });
        }
      }
    } catch (error) {
