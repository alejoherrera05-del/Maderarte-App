...
function renderItems(items) {
  ...
  return `<div class="od-items">${items.map(item => `<article class="od-item" data-order-item="${escapeHtml(item.id)}"><div class="od-item-copy"><strong>${escapeHtml(item.description || 'Producto')}</strong>${item.agreement ? `<span>${escapeHtml(item.fulfillment === 'PARA_SOLICITAR' ? 'Solicitar a fábrica' : item.agreement === 'ENTREGA_HOY' ? 'Entrega inmediata' : 'Separado / entregar después')}</span>` : ''}<span>${escapeHtml([item.reference, item.measures, item.fabricColor, item.woodColor, item.specifications].filter(Boolean).join(' · ') || 'Sin especificaciones adicionales')}</span></div><div class="od-item-amount"><strong>${escapeHtml(String(item.quantity ?? 0))}</strong><span>${escapeHtml(item.unit || 'UNIDAD')}</span><small>${escapeHtml(money(item.subtotal))}</small></div></article>`).join('')}</div>`;
}
...
function renderOrder(data) {
  ...
  const quoteLink = order.quoteOrigin ? `<a class="od-link" href="${escapeHtml(sandboxLink('/cotizacion-ver.html?cot=' + encodeURIComponent(order.quoteOrigin)))}">Cotización de origen ${escapeHtml(order.quoteOrigin)}</a>` : '';
  ...
  <main class="od-shell">
    <section class="od-hero"><div class="od-hero-top"><div><span class="od-kicker">Orden de pedido</span><h1>${escapeHtml(order.number || number)}</h1><p>${escapeHtml(order.client || 'Expediente comercial')}</p></div><div class="od-status-group"><span class="od-pill">${escapeHtml(humanizeCode(order.status))}</span><span class="od-pill">${escapeHtml(humanizeCode(order.productionStatus))}</span></div></div>${quoteLink}</section>
