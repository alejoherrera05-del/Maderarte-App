...
function companyMarkup(data) {
  ...
  const branchTitle = branch?.name || text('quote-meta-branch-name') || data.branchCode;
  const branchAddress = branch?.address || '';
  const branchReference = branch?.reference || '';
  const socials = `${COMPANY_PROFILE.socialNetworks.join(' · ')} ${COMPANY_PROFILE.socialHandle}`;

  return `<div class="quote-preview-company">
    <div>
      <strong>${escapeHtml(COMPANY_PROFILE.legalName)}</strong>
      <span>NIT ${escapeHtml(COMPANY_PROFILE.nit)}</span>
    </div>
    <div class="quote-preview-company-contact">
      <strong>${escapeHtml(branchTitle || 'Maderarte')}</strong>
      ${branchAddress ? `<span>${escapeHtml(branchAddress)}</span>` : ''}
      ${branchReference ? `<span>${escapeHtml(branchReference)}</span>` : ''}
      <div class="quote-preview-contact-row">
        <span><b>Cel.</b> ${escapeHtml(COMPANY_PROFILE.mobile)}</span>
        <span><b>WhatsApp</b> ${escapeHtml(COMPANY_PROFILE.whatsapp)}</span>
      </div>
      <span>${escapeHtml(COMPANY_PROFILE.website)}</span>
      <span class="quote-preview-social-line">${escapeHtml(socials)}</span>
    </div>
  </div>`;
}
...
function clientMarkup(client) {
  ...
  return `<div class="quote-preview-client-block">
    <div class="quote-preview-client-title"><span>Cliente</span>${client.name ? `<h3>${escapeHtml(client.name)}</h3>` : ''}</div>
...
function renderDocumentPreview() {
  ...
  const mainPage = `<section class="quote-preview-page quote-preview-main-page">
    <div class="quote-preview-letterhead">
      <div class="quote-preview-logo-lockup">
        <img src="/assets/brand/maderarte-logo-2026.webp" alt="Maderarte">
        <div><img src="/assets/brand/maderarte-wordmark-algerian.png" alt="MADERARTE"><span>${escapeHtml(COMPANY_PROFILE.slogan)}</span></div>
      </div>
      <div class="quote-preview-doc-id"><span>COTIZACIÓN</span><strong>${escapeHtml(data.number)}</strong>${data.date ? `<small>${escapeHtml(data.date)}</small>` : ''}</div>
    </div>
    ${companyMarkup(data)}
    <div class="quote-preview-context">
      ${data.branch?.name || data.branchCode ? `<span><small>Sede emisora</small><strong>${escapeHtml(data.branch?.name || data.branchCode)}</strong></span>` : ''}
      ${data.advisor ? `<span><small>Asesor</small><strong>${escapeHtml(data.advisor)}</strong></span>` : ''}
      <span><small>Estado</small><strong>Borrador</strong></span>
    </div>
    ${client}
