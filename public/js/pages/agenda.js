...
async function save(cancel) {
  ...
  if(!attempt){
    ...
    attempt={requestId:createRequestId('AGENDA'),payload:{id:state.edit?.id||'',revision:state.edit?.revision||0,number:state.order.order.number,date:$('ag-date').value,time:$('ag-time').value,items,notes:$('ag-notes').value,cancel}};
