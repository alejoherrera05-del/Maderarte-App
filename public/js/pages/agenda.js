...
await guardStandalonePage({permission:'agenda.read',render:async({session})=>{
  ...
  const number=new URLSearchParams(location.search).get('op');if(context.get('visita')){const e=state.events.find(x=>x.id===context.get('visita'));if(e){state.date=e.date;state.month=e.date.slice(0,7);render();openDetail(e);if(e.caseId){$('ag-back').href='/garantias.html?op='+encodeURIComponent(e.number)+'&case='+encodeURIComponent(e.caseId);$('ag-back').setAttribute('aria-label','Volver a la garantía');}}}if(number){$('ag-back').href=orderLink(number);$('ag-back').setAttribute('aria-label','Volver a la OP');if(state.enabled&&canOrders()){if(context.get('tipo')==='garantia'){await openEditor();chooseKind('GARANTIA');$('ag-task-fields').disabled=true;$('ag-task-save').disabled=true;$('ag-error').textContent='Recuperando el mueble…';try{await loadWarrantyVisit({number,itemId:context.get('item'),request:api});if(context.get('case')){const d=await api('GARANTIA_LISTAR',{number}),c=d.items.find(x=>x.id===context.get('case'));if(!c||c.itemId!==context.get('item'))throw Error('No se encontró el caso de esta visita.');state.caseId=c.id;$('ag-task-title').value=('Revisar '+c.piece).slice(0,160);$('ag-task-notes').value=c.issue;$('ag-assignee').value=c.assignee;$('ag-back').href='/garantias.html?op='+encodeURIComponent(number)+'&case='+encodeURIComponent(c.id);$('ag-back').setAttribute('aria-label','Volver a la garantía');}$('ag-task-fields').disabled=false;$('ag-task-save').disabled=false;$('ag-error').textContent='';$('ag-title').textContent='Programar revisión';}catch(error){$('ag-error').textContent=error.message;}}else await openEditor(null,number);}}
}});
