...
const scopedActions = new Set(['AJUSTE_PDF_LEER','AJUSTE_DOCUMENTOS_FINALIZAR','INTERNO_AJUSTE_DOCUMENTO_PREPARAR','INTERNO_AJUSTE_DOCUMENTO_CONFIRMAR','AJUSTE_CUENTA','AJUSTE_PREVISUALIZAR','AJUSTE_CONFIRMAR','AJUSTE_ESTADO','PRODUCCION_LISTAR','PRODUCCION_CUENTA','PRODUCCION_REGISTRAR','PRODUCCION_REGISTRO_ESTADO','REMISION_CAPACIDADES','REMISION_CUENTA','REMISION_CREAR','REMISION_CREACION_ESTADO','REMISION_OBTENER','REMISION_PDF_LEER','REMISION_DOCUMENTOS_FINALIZAR','INTERNO_REMISION_DOCUMENTO_PREPARAR','INTERNO_REMISION_DOCUMENTO_CONFIRMAR','COTIZACION_META','CLIENTES_LISTAR','CLIENTE_OBTENER','RECIBO_CAPACIDADES', 'RECIBO_CUENTA', 'RECIBO_CREAR', 'RECIBO_CREACION_ESTADO', 'RECIBO_OBTENER', 'RECIBO_PDF_LEER', 'RECIBO_DOCUMENTOS_FINALIZAR', 'INTERNO_RECIBO_DOCUMENTO_PREPARAR', 'INTERNO_RECIBO_DOCUMENTO_CONFIRMAR', 'ORDENES_LISTAR', 'ORDEN_CAPACIDADES','ORDEN_CREAR','ORDEN_CREACION_ESTADO','ORDEN_OBTENER','ORDEN_DOCUMENTOS_ESTADO','ORDEN_FOTO_GUARDAR','ORDEN_FOTO_LEER','ORDEN_PDF_LEER','ORDEN_DOCUMENTOS_FINALIZAR']);
scopedActions.add('COTIZACION_PREPARAR_PEDIDO');
for (const action of ['COTIZACION_CAPACIDADES','COTIZACION_CREAR','COTIZACION_CREACION_ESTADO','COTIZACION_OBTENER','COTIZACIONES_LISTAR','COTIZACION_DOCUMENTOS_ESTADO','COTIZACION_FOTO_GUARDAR','COTIZACION_FOTO_LEER','COTIZACION_PDF_LEER','COTIZACION_DOCUMENTOS_FINALIZAR']) scopedActions.add(action);
...
export function bindSandboxBanner(root, { prefill = false } = {}) {
  ...
  copy.textContent='Ensayo aislado. No cobra, no entrega y no consume consecutivos comerciales.';
  ...
  if (prefill) {
    ...
    const writeNote=root.querySelector('.quote-write-note');if(writeNote)writeNote.textContent='Solo registra una prueba en la hoja y carpeta aisladas. Revisa los archivos antes de autorizar su limpieza.';
  }
}
