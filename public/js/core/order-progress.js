...
const QUOTE_SAVE_STEPS = Object.freeze([
  ['prepare', 'Validar la propuesta', 'Cliente, sede, muebles, cantidades y valores'],
  ['record', 'Emitir cotización', 'Número oficial y registro único de la propuesta'],
  ['photos', 'Archivar referencias', 'Fotografías asociadas al mueble correcto'],
  ['document', 'Armar expediente', 'PDF y carpetas de la cotización'],
  ['verify', 'Confirmar expediente', 'Número, PDF, referencias y enlaces confirmados']
]);
...
const QUOTE_SAVE_MESSAGES = {
  ...
  photos: 'Cada referencia, en el mueble correcto.',
  document: 'Estoy armando el expediente de la propuesta.',
  verify: 'Una última revisión antes de terminar.'
};
...
export function createDocumentProgress({ kind = 'order', mode = 'save' } = {}, doc = document) {
  if (!['order', 'quote'].includes(kind) || !['save', 'preview'].includes(mode)) {
    throw new Error('Este proceso documental todavía no está disponible.');
  }
  const preview = mode === 'preview';
  const quoteSave = kind === 'quote' && mode === 'save';
  const noun = kind === 'quote' ? 'cotización' : 'pedido';
  const steps = preview ? PREVIEW_STEPS : quoteSave ? QUOTE_SAVE_STEPS : ORDER_PROGRESS_STEPS;
  ...
  } : quoteSave ? QUOTE_SAVE_MESSAGES : ORDER_SAVE_MESSAGES;
  const support = preview ? {
    ...
    prepare: 'Reviso cliente, sede, muebles, cantidades, acabados y valores antes de emitir nada.',
    record: 'Reservo el número oficial y registro esta misma propuesta una sola vez; la vista previa nunca consume consecutivos.',
    photos: 'Archivo cada fotografía en el mueble correcto para que las referencias no se mezclen.',
    document: 'Genero el PDF aprobado y organizo la cotización y sus referencias dentro del expediente del cliente.',
    verify: 'Compruebo número, PDF, fotografías y enlaces antes de dar la emisión por terminada.'
  } : {
  ...
  dialog.innerHTML = `<div class="order-progress-shell">
    ...
    <section class="order-progress-content">
      <p class="order-progress-eyebrow">${preview ? 'Preparando tu ' + noun : quoteSave ? 'Emitiendo tu cotización' : 'Guardando tu pedido'}</p>
      ...
      <button type="button" class="order-progress-return" hidden>Volver ${preview ? 'al documento' : quoteSave ? 'a la cotización' : 'al pedido'}</button>
    </section></div>`;
  ...
  function resetWait() {
    ...
    wait.textContent = preview
      ...
      : quoteSave
        ? 'Google todavía no confirma esta etapa. No vuelvas a pulsar Emitir; sigo con la misma cotización para evitar duplicados.'
  ...
  function hide() {
    ...
    else doc.querySelector('#order-save-status button, #quote-save-status button')?.focus({ preventScroll: true });
  }
  function begin() {
    if (destroyed) return;
    stop(); clock.textContent = ''; active = ''; numberNode.hidden = true; numberNode.textContent = '';
    resetWait(); message.textContent = support.prepare; caption.textContent = 'Revisión comercial en curso.';
  ...
  function update(event) {
    ...
    if (event.status === 'running') {
      ...
      active = event.step; dialog.querySelectorAll('[aria-current]').forEach(node => node.removeAttribute('aria-current')); row.setAttribute('aria-current', 'step');
  ...
  function pause(text) {
    ...
    const entry = steps.find(([id]) => id === active), row = entry && dialog.querySelector(`[data-progress-step="${active}"]`);
    ...
    title.textContent = preview ? 'No pude confirmar la vista previa todavía.' : quoteSave ? 'La emisión necesita una comprobación.' : 'La operación necesita una comprobación.';
    message.textContent = text || 'No se confirmó el resultado.';
    caption.textContent = preview ? 'Tu formulario sigue intacto.' : quoteSave ? 'Conservo la misma emisión para no duplicar la cotización.' : 'Conservo el mismo intento para no duplicar la venta.';
    wait.textContent = preview
      ? 'Vuelve al documento y revisa el aviso. No se ha emitido ninguna venta.'
      : quoteSave
        ? 'Vuelve a la cotización para consultar o retomar esta misma emisión. No generes otra propuesta.'
  ...
  function sync(state) {
    ...
    if (state.phase === 'confirmed') {
      title.textContent = preview ? 'Documento listo para tu revisión.' : quoteSave ? 'Cotización emitida. Expediente confirmado.' : 'Orden registrada. Todo quedó en su lugar.';
      hide();
  ...
  close.addEventListener('click', hide); win.addEventListener('pagehide', stop);
