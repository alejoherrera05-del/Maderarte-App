...
export function createOrderProgress(doc = document) {
  ...
  dialog.innerHTML = `<div class="order-progress-brand"><img src="/assets/brand/maderarte-logo-2026.webp" alt=""><span>MADERARTE<small>Tu pedido, paso a paso</small></span></div>
    <h2 id="order-progress-title" tabindex="-1">Estamos preparando tu pedido</h2>
    ...
    <div class="order-progress-foot"><img src="/assets/brand/maddy-by-maderarte.svg" alt="Maddy by Maderarte"><p data-progress-wait>Conserva esta pestaña abierta. No necesitas volver a pulsar Guardar.</p><span data-progress-time aria-hidden="true"></span></div>
  ...
  let timer = null, started = 0, busy = false, active = '', previousFocus;
  ...
  function open() {
    ...
    busy = true; close.hidden = true;
    if (timer === null) {
      started = Date.now();
      timer = win.setInterval(() => {
        const seconds = Math.floor((Date.now() - started) / 1000);
        clock.textContent = `${Math.floor(seconds / 60)}:${String(seconds % 60).padStart(2, '0')} transcurridos`;
        if (seconds >= 15) wait.textContent = 'Esta etapa aún no confirma su respuesta. Seguimos esperando; no repitas el guardado.';
      }, 1000);
    }
  }
  function hide() {
  ...
  function begin() {
    ...
    title.textContent = 'Estamos preparando tu pedido';
  ...
  function update(event) {
    ...
    if (event.number) setText(title, 'Pedido ' + event.number);
  }
  function pause(text) {
