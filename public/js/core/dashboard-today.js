...
export function mountToday(root, session) {
  ...
  const render = () => {
    ...
    root.innerHTML = `<header class="home-today-heading"><h2>Hoy en Maderarte</h2>${canRead?'<a href="/agenda.html">Ver agenda '+icon('arrow-right')+'</a>':''}</header>
      ...
      }).join(''):'<div class="home-empty">'+icon('calendar-dots')+'<strong>Todo al día</strong><p>No hay compromisos pendientes para hoy.</p><a href="/agenda.html">Ver próximos compromisos '+icon('arrow-right')+'</a></div>'}
