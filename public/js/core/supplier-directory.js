...
export function bindSupplierDirectory({uid,root,name,phone,onSelect}) {
 ...
 list.querySelectorAll('button').forEach(b=>b.addEventListener('click',()=>{const p=rows[Number(b.dataset.person)];name.value=p.name;phone.value=p.phone;favorite.checked=p.favorite;try{directory.save(p);status.textContent='Contacto seleccionado.';}catch{status.textContent='Contacto seleccionado. No se pudo actualizar recientes.';}onSelect();render();}));}
 ...
 name.addEventListener('input',()=>{const p=directory.list().find(p=>p.name.toLocaleLowerCase('es')===name.value.trim().toLocaleLowerCase('es'));favorite.checked=p?.favorite===true;render();});render();return {remember(){if(!name.value.trim())return;try{directory.save({name:name.value,phone:phone.value,favorite:favorite.checked});render();}catch{status.textContent='El mensaje está listo; este navegador no pudo guardar el contacto.';}}};
}
