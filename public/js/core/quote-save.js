import { finishQuoteDocuments } from './quote-media.js?v=quote-1';
...
export function quoteSaveCapabilitiesReady(value) {
  return value?.contractVersion === 1 && value.enabled === true && value.photosReady === true && value.documentsReady === true;
}
export function clearQuoteSaveSnapshots(storage) {
  for (const key of Object.keys(storage)) if (key.startsWith(QUOTE_SAVE_PREFIX)) storage.removeItem(key);
}
...
export function createQuoteSave({ uid, request, durable, temporary, locks, crypto, activeUid = () => uid, onState = () => {}, onProgress = () => {}, scope = '' }) {
  ...
  const progress = event => { try { onProgress(event); } catch { /* feedback never changes persistence */ } };
  let busy = false, supportsMedia = false, state = { phase: 'disabled', canSave: false, locked: false, message: '' };
  const notify = (phase, options = {}) => { state = { phase, canSave: false, locked: true, message: '', ...options }; onState(copy(state)); return copy(state); };
  ...
  const getJournal = () => {
    const raw = durable.getItem(key); if (!raw) return null;
    let value; try { value = JSON.parse(raw); } catch { /* fail closed */ }
  ...
  const storedPayload = async journal => {
    let snapshot; try { snapshot = JSON.parse(temporary.getItem(key) || 'null'); } catch { return null; }
  ...
  const owns = journal => { try { const saved = JSON.parse(temporary.getItem(key) || 'null'); return saved?.uid === uid && saved.requestId === journal.requestId; } catch { return false; } };
  const confirmed = journal => notify('confirmed', { number: journal.number, requestId: journal.requestId, ownsDraft: owns(journal),
    message: `Cotización ${journal.number} emitida. Puedes abrirla sin volver a registrarla.` });
  const uncertain = journal => notify('uncertain', { requestId: journal.requestId,
    message: 'Falta confirmar la emisión. Consulta este mismo intento; no generes otra cotización.' });
  const documentsPending = (journal, message, working = false) => notify('documents', { working, number: journal.number, requestId: journal.requestId, ownsDraft: owns(journal),
    message: message || `Cotización ${journal.number} registrada. Falta completar su expediente documental; no se emitirá otra.` });

  async function completeDocuments(journal) {
    progress({ step: 'prepare', status: 'complete', message: 'Datos de la cotización emitida recuperados.' });
    progress({ step: 'record', status: 'complete', number: journal.number, message: 'El número ya está confirmado; no se volverá a consumir un consecutivo.' });
    documentsPending(journal, 'Completando fotografías y PDF de la cotización emitida…', true);
    try {
      sameUser();
      const payload = await storedPayload(journal);
      await finishQuoteDocuments(journal.number, payload?._media || [], async (...args) => { sameUser(); const response = await request(...args); sameUser(); return response; },
        message => documentsPending(journal, message, true), progress);
      const receipt = { ...journal, stage: 'confirmed' }; put(durable, receipt);
      if (owns(journal)) { try { put(temporary, { uid, requestId: journal.requestId, confirmed: true }); } catch { /* durable receipt wins */ } }
      return confirmed(receipt);
    } catch (error) { return documentsPending(journal, error?.message); }
  }
  async function accept(data, journal) {
    ...
    if (owns(journal)) { try { put(temporary, { uid, requestId: journal.requestId, confirmed: true }); } catch { /* confirmed marker prevents resubmission */ } }
    return confirmed(receipt);
  }
  async function capabilities() {
    sameUser(); const response = await request('COTIZACION_CAPACIDADES', {}); sameUser();
    supportsMedia = response?.data?.mediaWorkflow === 1;
    return quoteSaveCapabilitiesReady(response?.data);
  }
  async function check(journal) {
    ...
    notify('checking', { requestId: journal.requestId, message: 'Consultando la misma emisión de cotización…' });
    ...
    if (data?.saved === false && data.requestId === journal.requestId && data.state === 'NO_CONFIRMADO' && data.retrySameRequest === true && await storedPayload(journal)) {
      return notify('retry', { requestId: journal.requestId, message: 'No hay confirmación. Puedes reenviar exactamente este mismo intento.' });
    }
  ...
  async function exclusive(task) {
    if (busy) return copy(state); busy = true;
    try {
      sameUser();
      ...
      return await locks.request(key, { mode: 'exclusive', ifAvailable: true }, async lock => !lock
        ? notify('other-tab', { message: 'Esta cotización se está revisando en otra pestaña. Consulta el resultado antes de continuar.' })
        : task());
    } catch (error) {
      let journal; try { journal = getJournal(); } catch { return unavailable(error); }
      return journal?.stage === 'confirmed' ? confirmed(journal) : journal?.stage === 'documents' ? documentsPending(journal) : journal ? uncertain(journal) : unavailable(error, false);
    } finally { busy = false; }
  }
  async function refresh() {
    return exclusive(async () => {
      const journal = getJournal(); if (journal) return check(journal);
      ...
      return notify(enabled ? 'ready' : 'disabled', { canSave: enabled, mediaEnabled: supportsMedia, locked: false,
        message: enabled ? '' : 'La emisión real de cotizaciones continúa en preparación.' });
    });
  }
  async function transmit(payload, journal, firstAttempt = false) {
    notify('saving', { requestId: journal.requestId, message: 'Emitiendo la cotización…' });
    progress({ step: 'record', status: 'running', message: 'Estoy reservando el número oficial y registrando la propuesta una sola vez.' });
    sameUser();
    try {
  ...
  async function save(payload) {
    let snapshot; try { snapshot = copy(payload); } catch { return unavailable(); }
    return exclusive(async () => {
      const journal = getJournal(); if (journal) return check(journal);
      if (!await capabilities()) return notify('disabled', { locked: false, message: 'La emisión real todavía no está habilitada. Conserva el borrador.' });
      if (snapshot._media?.length && !supportsMedia) return notify('disabled', { locked: false, message: 'El servidor todavía no admite fotografías de cotización. No se emitió nada.' });
      const pending = { version: 1, uid, requestId: `QT-${crypto.randomUUID()}`, digest: await digest(snapshot), stage: 'pending' };
      put(temporary, { uid, requestId: pending.requestId, payload: snapshot }); put(durable, pending);
  ...
  async function startNew(beforeClear = () => {}) {
    return exclusive(async () => {
      const journal = getJournal(); if (!journal || journal.stage !== 'confirmed') return journal ? uncertain(journal) : unavailable();
      await beforeClear(); temporary.removeItem(key); durable.removeItem(key); return notify('new', { locked: false });
    });
  }
