...
export function requestTimeoutMs(action, sandbox = false, override) {
  return Math.max(1_000, Number(override || (sandbox || /^(ORDEN(?:ES)?|COTIZACION(?:ES)?|RECIBO|REMISION|PRODUCCION|CLIENTES?|SISTEMA|USUARIOS|DASHBOARD|AGENDA|GARANTIA|RECAUDOS?)_/.test(action) ? 120_000 : APP_CONFIG.requestTimeoutMs)));
}
