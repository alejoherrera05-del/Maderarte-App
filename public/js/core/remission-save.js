...
export function createRemissionSave(options) {
  ...
  const messages={preparing:'Comprobando disponibilidad del despacho…',confirmed:'Despacho registrado. Puedes abrir su remisión.',documents:'Despacho registrado. Falta completar su PDF; las cantidades ya están registradas.',saving:'Confirmando el despacho…',checking:'Consultando el resultado del despacho…',uncertain:'Falta confirmar el resultado. Consulta el mismo intento antes de otro despacho.'};
  return createOrderSave({...options,kind:'remission',onState:state=>options.onState({...state,message:(state.phase==='documents'&&state.working ? state.message.replace('Completando los archivos de la orden registrada…','Despacho registrado. Generando y archivando el PDF…') : messages[state.phase])||state.message.replaceAll('Pedido','Despacho').replaceAll('pedido','despacho').replaceAll('la orden','la remisión')}),
    ...
    finishDocuments:async(number,media,request,status)=>{
      ...
      status('PDF generado. Verificando el archivo de la remisión…');
